...
function resolveBrowserBaseUrl(params: {
  ...
  if (!resolved.enabled) {
    throw new Error(
      "Browser control is disabled. Set browser.enabled=true in ~/.quantclaw/quantclaw.json.",
    );
  }
  return undefined;
}
...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via QuantClaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated QuantClaw-managed browser (`quantclaw`).",
