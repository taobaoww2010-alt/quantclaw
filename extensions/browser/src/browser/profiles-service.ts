...
import type { BrowserProfileConfig, QuantClawConfig } from "../config/config.js";
import { loadConfig, writeConfigFile } from "../config/config.js";
import { deriveDefaultBrowserCdpPortRange } from "../config/port-defaults.js";
import { resolveUserPath } from "../utils.js";
import { resolveQuantClawUserDataDir } from "./chrome.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const createProfile = async (params: CreateProfileParams): Promise<CreateProfileResult> => {
    ...
    const nextConfig: QuantClawConfig = {
      ...cfg,
      browser: {
        ...cfg.browser,
        profiles: {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback && resolved.driver === "quantclaw") {
      try {
      ...
      const userDataDir = resolveQuantClawUserDataDir(name);
