...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "quantclaw";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "quantclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "quantclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (quantclaw|existing-session). Default: quantclaw")
    .action(
      async (
        opts: {
          name: string;
