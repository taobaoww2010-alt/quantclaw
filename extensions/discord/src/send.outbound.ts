...
import { maxBytesForKind } from "quantclaw/plugin-sdk/media-runtime";
...
import { loadWebMediaRaw } from "quantclaw/plugin-sdk/web-media";
...
async function materializeVoiceMessageInput(mediaUrl: string): Promise<{ filePath: string }> {
  ...
  const tempDir = resolvePreferredQuantClawTmpDir();
