...
`quantclaw.invoke --each`) without adding new QuantClaw code per workflow.
...
This extension depends on QuantClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** QuantClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.quantclaw/extensions` as a standalone plugin directory.
