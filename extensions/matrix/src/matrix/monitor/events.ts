...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another QuantClaw runtime is using the same account. " +
    `Check 'quantclaw matrix verify status --verbose --account ${accountId}' and 'quantclaw matrix devices list --account ${accountId}'.`
  );
}
