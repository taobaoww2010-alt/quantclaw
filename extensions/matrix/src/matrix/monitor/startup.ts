...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleQuantClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale QuantClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleQuantClawDevices.map((device) => device.deviceId).join(", ")}. Run 'quantclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (startupVerification.kind === "verified") {
      ...
      params.logger.info(
        "matrix: device not verified — run 'quantclaw matrix verify device <key>' to enable E2EE",
      );
