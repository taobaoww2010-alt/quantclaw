...
metadata: { "quantclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## QuantClaw Runtime Mapping

- **Task tool** in the upstream spec == QuantClaw `sessions_spawn`
- **File I/O** == QuantClaw `read`/`write`
- **Remote fetch** == QuantClaw `web_fetch` (or `exec` with curl when POST is required)
