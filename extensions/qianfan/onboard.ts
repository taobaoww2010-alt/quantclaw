...
function resolveQianfanPreset(cfg: QuantClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: QuantClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: QuantClawConfig): QuantClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: QuantClawConfig): QuantClawConfig {
