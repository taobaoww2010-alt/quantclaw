...
export function listQQBotAccountIds(cfg: QuantClawConfig): string[] {
...
export function resolveDefaultQQBotAccountId(cfg: QuantClawConfig): string {
  const qqbot = cfg.channels?.qqbot as QQBotChannelConfig | undefined;
  if (qqbot?.appId || process.env.QQBOT_APP_ID) {
...
/** Apply account config updates back into the QuantClaw config object. */
