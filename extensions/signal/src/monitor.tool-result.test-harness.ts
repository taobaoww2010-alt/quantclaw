...
vi.mock("quantclaw/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/quantclaw-sessions.json"),
...
vi.mock("quantclaw/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantclaw/plugin-sdk/conversation-runtime")>(
    "quantclaw/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("quantclaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantclaw/plugin-sdk/security-runtime")>(
    "quantclaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("quantclaw/plugin-sdk/channel-runtime", async () => {
  const actual = await vi.importActual<typeof import("quantclaw/plugin-sdk/channel-runtime")>(
    "quantclaw/plugin-sdk/channel-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("quantclaw/plugin-sdk/reply-runtime"),
      import("quantclaw/plugin-sdk/channel-runtime"),
    ]);
    resetInboundDedupe();
    config = {
