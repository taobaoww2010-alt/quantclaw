...
import { getChatChannelMeta, type ChannelPlugin, type QuantClawConfig } from "./runtime-api.js";

export const SLACK_CHANNEL = "slack" as const;

function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "QuantClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for QuantClaw`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/quantclaw",
          description: "Send a message to QuantClaw",
          should_escape: false,
        },
      ],
    },
...
export function buildSlackSetupLines(botName = "QuantClaw"): string[] {
  return [
