...
function getChannelConfig(cfg: QuantClawConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: QuantClawConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: QuantClawConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: QuantClawConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL QuantClaw uses to send replies back to Chat.",
      ],
