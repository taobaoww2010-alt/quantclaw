...
import { info, success } from "quantclaw/plugin-sdk/runtime-env";
...
import type { WebChannel } from "quantclaw/plugin-sdk/text-runtime";
import { resolveUserPath } from "quantclaw/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("quantclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
