...
function mergeWhatsAppConfig(
  cfg: QuantClawConfig,
  patch: Partial<NonNullable<NonNullable<QuantClawConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: QuantClawConfig, dmPolicy: DmPolicy): QuantClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: QuantClawConfig, allowFrom?: string[]): QuantClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: QuantClawConfig, selfChatMode: boolean): QuantClawConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: QuantClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so QuantClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for QuantClaw" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("quantclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
