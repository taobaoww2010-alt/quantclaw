...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("quantclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("quantclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced quantclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced quantclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
