...
async function main() {
  const stateDir = process.env.QUANTCLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".quantclaw");
  const configPath =
    process.env.OPENCLAW_CONFIG_PATH?.trim() || path.join(stateDir, "quantclaw.json");
