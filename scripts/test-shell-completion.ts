 ...
 * `quantclaw update`. Use it to verify the completion installation flow
...
\`quantclaw update\`, \`quantclaw doctor\`, and \`quantclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# QuantClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`quantclaw completion --install\` later to enable.`));
    return;
  }
