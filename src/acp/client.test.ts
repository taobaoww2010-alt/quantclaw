...
const createTempDir = () => tempDirs.make("quantclaw-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets QUANTCLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "quantclaw",
    });

    expect(env.QUANTCLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("quantclaw");
  });

  it("overrides pre-existing QUANTCLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      QUANTCLAW_SHELL: "wrong",
    });
    expect(env.QUANTCLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves QUANTCLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        QUANTCLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["QUANTCLAW_SHELL", openAiApiKeyEnv]) },
    );
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "quantclaw",
        serverArgs: ["acp"],
        defaultServerCommand: "quantclaw",
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "quantclaw", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "quantclaw", "dist", "entry.js");
    const shimPath = path.join(dir, "quantclaw.cmd");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/quantclaw-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/quantclaw-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/quantclaw-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
