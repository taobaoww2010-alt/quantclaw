...
import { ensureQuantClawCliOnPath } from "../infra/path-env.js";
import {
...
export function resolveAcpClientSpawnEnv(
  ...
  env.QUANTCLAW_SHELL = "acp-client";
  return env;
}
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "quantclaw";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "quantclaw-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("☯️ QuantClaw ACP client");
