...
export function listAgentEntries(cfg: QuantClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: QuantClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: QuantClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
export function resolveAgentModelPrimary(cfg: QuantClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: QuantClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
