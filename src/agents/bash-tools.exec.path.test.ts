...
describe("exec PATH login shell merge", () => {
  ...
  it("sets QUANTCLAW_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }
    ...
    const result = await tool.execute("call-openclaw-shell", {
      command: 'printf "%s" "${QUANTCLAW_SHELL:-}"',
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantclaw-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/quantclaw-ssl-keys.log";
    try {
      ...
      expect(output).not.toContain("/tmp/quantclaw-ssl-keys.log");
    } finally {
