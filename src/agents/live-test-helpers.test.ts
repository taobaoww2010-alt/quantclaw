...
describe("isLiveTestEnabled", () => {
  it("treats LIVE and QUANTCLAW_LIVE_TEST as shared live gates", () => {
    expect(isLiveTestEnabled([], { LIVE: "1" })).toBe(true);
    expect(isLiveTestEnabled([], { QUANTCLAW_LIVE_TEST: "1" })).toBe(true);
...
describe("isLiveProfileKeyModeEnabled", () => {
  it("only enables profile-key mode for the dedicated flag", () => {
    ...
    expect(isLiveProfileKeyModeEnabled({ QUANTCLAW_LIVE_TEST: "1" })).toBe(false);
