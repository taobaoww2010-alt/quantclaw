...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "quantclaw-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.QUANTCLAW_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.QUANTCLAW_AUTH_STORE_READONLY;
        } else {
          process.env.QUANTCLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
