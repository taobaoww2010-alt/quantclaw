...
import { createQuantClawTools } from "./quantclaw-tools.js";
...
function resolveExecConfig(params: { cfg?: QuantClawConfig; agentId?: string }) {
...
export function createQuantClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createQuantClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
