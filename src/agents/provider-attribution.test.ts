...
describe("provider attribution", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { QUANTCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        ...
        version: "2026.3.22",
        "User-Agent": "quantclaw/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { QUANTCLAW_VERSION: "2026.3.22" })).toEqual(
      {
        originator: "quantclaw",
        version: "2026.3.22",
        "User-Agent": "quantclaw/2026.3.22",
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { QUANTCLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ QUANTCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
