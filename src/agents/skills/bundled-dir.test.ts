...
describe("resolveBundledSkillsDir", () => {
  ...
  it("returns OPENCLAW_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantclaw-bundled-override-"));
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantclaw-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "quantclaw" }));
