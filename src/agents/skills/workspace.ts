...
import { resolveQuantClawMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: QuantClawConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "quantclaw-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "quantclaw-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveQuantClawMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: QuantClawConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`quantclaw skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`quantclaw skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
