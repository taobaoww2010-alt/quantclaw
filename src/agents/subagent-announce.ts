    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to QuantClaw sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`quantclaw ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for QuantClaw subagents (`runtime: "subagent"`).',
