...
function resolveAttachmentLimits(config: QuantClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".quantclaw", "attachments");
  const relDir = path.posix.join(".quantclaw", "attachments", attachmentId);
