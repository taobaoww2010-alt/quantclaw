...
type CoreToolDefinition = {
  ...
  includeInQuantClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const quantclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInQuantClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:quantclaw": quantclawTools,
