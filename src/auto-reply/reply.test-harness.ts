...
type HomeEnvSnapshot = {
  ...
  QUANTCLAW_STATE_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    QUANTCLAW_STATE_DIR: process.env.QUANTCLAW_STATE_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".quantclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.QUANTCLAW_STATE_DIR = path.join(home, ".quantclaw");
    process.env.QUANTCLAW_AGENT_DIR = path.join(home, ".quantclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".quantclaw", "agent");
