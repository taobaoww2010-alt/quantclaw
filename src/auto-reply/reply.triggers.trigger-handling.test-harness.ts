...
const piEmbeddedMocks = getSharedMocks("quantclaw.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("quantclaw.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks(
  "quantclaw.trigger-handling.model-fallback-mocks",
  () => ({
...
const webSessionMocks = getSharedMocks("quantclaw.trigger-handling.web-session-mocks", () => ({
...
type TempHomeEnvSnapshot = {
  ...
  quantclawHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    quantclawHome: process.env.QUANTCLAW_HOME,
    stateDir: process.env.QUANTCLAW_STATE_DIR,
  };
}
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("QUANTCLAW_HOME", snapshot.quantclawHome);
  restoreKey("QUANTCLAW_STATE_DIR", snapshot.stateDir);
}

function setTempHomeEnv(home: string): void {
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  delete process.env.QUANTCLAW_HOME;
  process.env.QUANTCLAW_STATE_DIR = join(home, ".quantclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "quantclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".quantclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): QuantClawConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "quantclaw"),
