...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("quantclaw-daemon-install-int-");
    configPath = path.join(tempHome, "quantclaw.json");
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.QUANTCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
