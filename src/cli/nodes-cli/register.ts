    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw nodes status", "List known nodes with live status."],
          ["quantclaw nodes pairing pending", "Show pending node pairing requests."],
          [
            'quantclaw nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["quantclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.quantclaw.ai/cli/nodes")}\n`,
    );
