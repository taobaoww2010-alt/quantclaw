...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.quantclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  const stateDir = env.QUANTCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.QUANTCLAW_STATE_DIR?.trim()) {
    env.QUANTCLAW_STATE_DIR = stateDir;
  }

  if (!env.OPENCLAW_CONFIG_PATH?.trim()) {
    env.OPENCLAW_CONFIG_PATH = path.join(stateDir, "quantclaw.json");
  }
