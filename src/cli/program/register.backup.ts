...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.quantclaw.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw backup create", "Create a timestamped backup in the current directory."],
          [
            "quantclaw backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "quantclaw backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "quantclaw backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "quantclaw backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["quantclaw backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          [
            "quantclaw backup verify ~/Backups/latest.tar.gz --json",
