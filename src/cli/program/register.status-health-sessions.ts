...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw status", "Show channel health + session summary."],
          ["quantclaw status --all", "Full diagnosis (read-only)."],
          ["quantclaw status --json", "Machine-readable output."],
          ["quantclaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "quantclaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["quantclaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.quantclaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.quantclaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw sessions", "List all sessions."],
          ["quantclaw sessions --agent work", "List sessions for one agent."],
          ["quantclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["quantclaw sessions --active 120", "Only last 2 hours."],
          ["quantclaw sessions --json", "Machine-readable output."],
          ["quantclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.quantclaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "quantclaw sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["quantclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["quantclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["quantclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "quantclaw sessions cleanup --enforce --store ./tmp/sessions.json",
