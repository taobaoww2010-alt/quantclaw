...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw security audit", "Run a local security audit."],
          ["quantclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["quantclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "quantclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["quantclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["quantclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.quantclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("☯️ QuantClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("quantclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("quantclaw security audit --fix")}`));
        if (!fixResult) {
