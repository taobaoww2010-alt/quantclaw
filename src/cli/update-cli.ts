...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["quantclaw update", "Update a source checkout (git)"],
        ["quantclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["quantclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["quantclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["quantclaw update --tag main", "One-off package install from GitHub main"],
        ["quantclaw update --dry-run", "Preview actions without changing anything"],
        ["quantclaw update --no-restart", "Update without restarting the service"],
        ["quantclaw update --json", "Output result as JSON"],
        ["quantclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["quantclaw update wizard", "Interactive update wizard"],
        ["quantclaw --update", "Shorthand for quantclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run quantclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.quantclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantclaw update status", "Show channel + version status."],
          ["quantclaw update status --json", "JSON output."],
          ["quantclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.quantclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
