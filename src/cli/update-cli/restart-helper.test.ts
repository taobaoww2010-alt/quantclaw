...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'quantclaw-gateway.service'");
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "☯️ QuantClaw Gateway"');
      expect(content).toContain('schtasks /Run /TN "☯️ QuantClaw Gateway"');
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCLAW_PROFILE: "default",
        OPENCLAW_WINDOWS_TASK_NAME: "☯️ QuantClaw Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "☯️ QuantClaw Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "☯️ QuantClaw Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      ...
      expect(content).toContain("quantclaw-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "☯️ QuantClaw Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
