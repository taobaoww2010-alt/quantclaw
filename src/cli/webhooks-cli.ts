...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.quantclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + QuantClaw hooks")
    ...
    .option("--hook-url <url>", "☯️ QuantClaw hook URL")
    .option("--hook-token <token>", "☯️ QuantClaw hook token")
