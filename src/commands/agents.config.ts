...
function resolveAgentName(cfg: QuantClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: QuantClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: QuantClawConfig): AgentSummary[] {
