...
const SEARCH_TOOL = "https://docs.quantclaw.ai/mcp.SearchQuantClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.quantclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('quantclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.quantclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('quantclaw docs "your query"')}`);
    }
    return;
  }
