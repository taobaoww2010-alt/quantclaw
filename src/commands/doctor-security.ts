...
import type { QuantClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: QuantClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: QuantClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("quantclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.quantclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("quantclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.quantclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("quantclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("quantclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("quantclaw doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "quantclaw config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("quantclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('quantclaw config set session.dmScope "per-channel-peer"') +
