...
export const resolveQuantClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadQuantClawPlugins: () => createEmptyPluginRegistry(),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/quantclaw.json",
    createConfigIO,
...
vi.mock("../infra/quantclaw-root.js", () => ({
...
beforeEach(() => {
  ...
  resolveQuantClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.QUANTCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.QUANTCLAW_UPDATE_IN_PROGRESS;
  process.env.QUANTCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantclaw-doctor-state-"));
  process.env.QUANTCLAW_STATE_DIR = tempStateDir;
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    delete process.env.QUANTCLAW_STATE_DIR;
  } else {
    process.env.QUANTCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.QUANTCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.QUANTCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
