...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.QUANTCLAW_DEBUG_HEALTH)) {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: QuantClawConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: QuantClawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.QUANTCLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
