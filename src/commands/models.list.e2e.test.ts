...
const ensureQuantClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveQuantClawAgentDir = vi.fn().mockReturnValue("/tmp/quantclaw-agent");
...
beforeEach(() => {
  ...
  ensureQuantClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureQuantClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureQuantClawModelsJson).toHaveBeenCalled();
    expect(ensureQuantClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
