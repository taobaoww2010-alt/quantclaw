...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByQuantClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByQuantClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("☯️ QuantClaw status"));
  runtime.log("");
  ...
  if (summary.taskAudit.errors > 0) {
    runtime.log("");
    runtime.log(
      theme.muted(`Task maintenance: ${formatCliCommand("quantclaw tasks maintenance --apply")}`),
    );
  }
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`quantclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("quantclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("quantclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("quantclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("quantclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.quantclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.quantclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("quantclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("quantclaw logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("quantclaw node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("quantclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("quantclaw gateway probe")}`);
  }
}
