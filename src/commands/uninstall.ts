...
async function removeMacApp(runtime: RuntimeEnv, dryRun?: boolean) {
  ...
  await removePath("/Applications/QuantClaw.app", runtime, {
    dryRun,
    label: "/Applications/QuantClaw.app",
  });
}

function logBackupRecommendation(runtime: RuntimeEnv) {
  runtime.log(`Recommended first: ${formatCliCommand("quantclaw backup create")}`);
}
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.quantclaw" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/QuantClaw.app",
        },
      ],
