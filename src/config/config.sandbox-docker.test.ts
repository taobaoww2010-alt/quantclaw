...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("quantclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "quantclaw-sandbox-browser-global" },
      agentBrowser: { network: "quantclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("quantclaw-sandbox-browser-agent");
  });
