...
import { resetConfigRuntimeState, type QuantClawConfig } from "./config.js";
...
export async function writeQuantClawConfig(home: string, config: unknown): Promise<string> {
  const configPath = path.join(home, ".quantclaw", "quantclaw.json");
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.QUANTCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected QUANTCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeQuantClawConfig(home, config);
