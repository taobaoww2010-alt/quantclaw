...
import type { QuantClawConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.quantclaw.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: QuantClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: QuantClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: QuantClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = QuantClawSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = validated.data as QuantClawConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; QuantClaw mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: QuantClawConfig | null | undefined;

  const ensureCompatConfig = (): QuantClawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native QuantClaw config schema.
