...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.QUANTCLAW_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.quantclaw/agents/<agentId>/agent/).
  const agentId = normalizedRequested ?? defaultAgentId;
  const agentCfg = buildCronAgentDefaultsConfig({
    defaults: params.cfg.agents?.defaults,
    agentConfigOverride,
  });
  const cfgWithAgentDefaults: QuantClawConfig = {
    ...params.cfg,
  ...
  try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: async (providerOverride, modelOverride, runOptions) => {
          ...
          if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
            ...
            // the configured timeoutSeconds. See: https://github.com/quantclaw/quantclaw/issues/29774
          ...
          const result = await runEmbeddedPiAgent({
            ...
            // inherit owner-only tooling like local `quantclaw agent` runs.
            senderIsOwner: true,
            messageChannel,
    ...
    // See: https://github.com/quantclaw/quantclaw/issues/57206
    while (true) {
      try {
