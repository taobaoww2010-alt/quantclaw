...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds QUANTCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `quantclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service QUANTCLAW_GATEWAY_TOKEN does not match gateway.auth.token in quantclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.QUANTCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.QUANTCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `quantclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
