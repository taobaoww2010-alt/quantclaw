...
function loadGatewayConfig(): QuantClawConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: QuantClawConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__QUANTCLAW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
