...
export function resolveHooksConfig(cfg: QuantClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: QuantClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-quantclaw-token"] === "string"
      ? req.headers["x-quantclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-quantclaw-idempotency-key"]) ||
