...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-QuantClaw-Token header (query parameters are not allowed).",
      );
      return true;
    }
        ...
        wss.handleUpgrade(req, socket, head, (ws) => {
          (
            ws as unknown as import("ws").WebSocket & {
              __quantclawPreauthBudgetClaimed?: boolean;
              __quantclawPreauthBudgetKey?: string;
            }
          ).__quantclawPreauthBudgetKey = preauthBudgetKey;
          ...
          const budgetClaimed = Boolean(
            ...
            ).__quantclawPreauthBudgetClaimed,
          );
          if (budgetClaimed) {
            budgetTransferred = true;
