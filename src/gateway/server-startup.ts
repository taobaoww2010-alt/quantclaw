...
import type { loadQuantClawPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureQuantClawModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadQuantClawPlugins>;
  ...
  // Tests can opt out via QUANTCLAW_SKIP_CHANNELS (or legacy QUANTCLAW_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.QUANTCLAW_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.QUANTCLAW_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (QUANTCLAW_SKIP_CHANNELS=1 or QUANTCLAW_SKIP_PROVIDERS=1)",
    );
  }
