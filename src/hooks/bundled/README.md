...
This directory contains hooks that ship with QuantClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.quantclaw/workspace`)

**Enable**:

```bash
quantclaw hooks enable session-memory
```
...
```bash
quantclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.quantclaw/logs/commands.log`

**Enable**:

```bash
quantclaw hooks enable command-logger
```
...
```bash
quantclaw hooks enable boot-md
```
...
homepage: https://docs.quantclaw.ai/automation/hooks#my-hook
metadata:
  { "quantclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.quantclaw/hooks/` (shared across workspaces)
...
```bash
quantclaw hooks list
```

Show hook details:

```bash
quantclaw hooks info session-memory
```

Check hook status:

```bash
quantclaw hooks check
```
...
```bash
quantclaw hooks enable session-memory
quantclaw hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.quantclaw/quantclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'quantclaw.*gateway' && pnpm quantclaw gateway`
3. Enable the hook: `quantclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.quantclaw.ai/automation/hooks
