...
homepage: https://docs.quantclaw.ai/automation/hooks#command-logger
metadata:
  {
    "quantclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.quantclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.quantclaw/logs/commands.log ~/.quantclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/quantclaw`:
   ```
   /home/username/.quantclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.quantclaw/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.quantclaw/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.quantclaw/logs/commands.log | jq .
```
