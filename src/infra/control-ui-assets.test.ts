...
vi.mock("./openclaw-root.js", () => ({
  resolveQuantClawPackageRoot: vi.fn(async () => null),
  resolveQuantClawPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveQuantClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openclaw");
    (
      openclawRoot.resolveQuantClawPackageRoot as unknown as ReturnType<typeof vi.fn>
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "quantclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "quantclaw.mjs"))).resolves.toBe(
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    (
      openclawRoot.resolveQuantClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
