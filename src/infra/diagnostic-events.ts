...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __quantclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__quantclawDiagnosticEventsState) {
    globalStore.__quantclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__quantclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: QuantClawConfig): boolean {
