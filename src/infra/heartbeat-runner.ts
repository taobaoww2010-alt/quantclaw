...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: QuantClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: QuantClawConfig) {
...
function resolveHeartbeatAgents(cfg: QuantClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: QuantClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: QuantClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: QuantClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
