...
const TEMP_DIR_PREFIX = "quantclaw-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("quantclaw-install-source-utils-");
  const archivePath = path.join(cwd, "quantclaw-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("quantclaw-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "quantclaw-plugin@1.2.3",
          name: "quantclaw-plugin",
          version: "1.2.3",
          filename: "quantclaw-plugin-1.2.3.tgz",
    ...
    const result = await runPack("quantclaw-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "quantclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "quantclaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "quantclaw-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "quantclaw-plugin-1.2.3.tgz");
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"quantclaw-plugin@1.2.3","name":"quantclaw-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "quantclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "quantclaw-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    expectPackError(result, [
      ...
      "@openclaw/whatsapp",
      "docs.quantclaw.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("quantclaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "quantclaw-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            ...
            filename: "quantclaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "quantclaw-plugin-demo-2.0.0.tgz"),
      metadata: {
