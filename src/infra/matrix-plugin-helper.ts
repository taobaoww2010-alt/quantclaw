...
export const MATRIX_LEGACY_CRYPTO_INSPECTOR_UNAVAILABLE_MESSAGE =
  "Legacy Matrix encrypted state was detected, but the Matrix plugin helper is unavailable. Install or repair @quantclaw/matrix so QuantClaw can inspect the old rust crypto store before upgrading.";
...
export async function loadMatrixLegacyCryptoInspector(params: {
  ...
  if (resolution.status === "unsafe") {
    throw new Error(
      `Matrix plugin helper path is unsafe: ${resolution.candidatePath}. Reinstall @quantclaw/matrix and try again.`,
    );
  }
  ...
  const pending = (async () => {
    ...
    if (!inspectLegacyMatrixCryptoStore) {
      throw new Error(
        `Matrix plugin helper at ${helperPath} does not export inspectLegacyMatrixCryptoStore(). Reinstall @quantclaw/matrix and try again.`,
      );
    }
