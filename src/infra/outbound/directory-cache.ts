...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: QuantClawConfig | null = null;
  ...
  get(key: string, cfg: QuantClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: QuantClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: QuantClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: QuantClawConfig): void {
