...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/quantclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/quantclaw-state", "devices"),
      pendingPath: path.join("/tmp/quantclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/quantclaw-state", "devices", "paired.json"),
    });
  });
