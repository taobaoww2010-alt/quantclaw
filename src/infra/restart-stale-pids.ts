 ...
 * returning control to the caller (typically `triggerQuantClawRestart` →
...
/**
 * Parse quantclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("quantclaw")) {
 ...
 * Returns only PIDs that belong to quantclaw gateway processes (not the current process).
 */
