...
export const POSIX_QUANTCLAW_TMP_DIR = "/tmp/quantclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredQuantClawTmpDirOptions = {
...
export function resolvePreferredQuantClawTmpDir(
  options: ResolvePreferredQuantClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "quantclaw" : `quantclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[quantclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback QuantClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback QuantClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_QUANTCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_QUANTCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_QUANTCLAW_TMP_DIR)) {
      return POSIX_QUANTCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_QUANTCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_QUANTCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_QUANTCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_QUANTCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_QUANTCLAW_TMP_DIR;
