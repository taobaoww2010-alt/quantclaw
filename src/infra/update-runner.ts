...
const DEFAULT_PACKAGE_NAME = "quantclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["quantclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "quantclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "quantclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("quantclaw doctor", doctorArgv, gitRoot, { OPENCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
