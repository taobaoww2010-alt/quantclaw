...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*quantclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "quantclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "quantclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "quantclaw-gw");
  ...
  records.push(`_quantclaw-gw._tcp IN PTR ${instanceLabel}._quantclaw-gw._tcp`);
  records.push(`${instanceLabel}._quantclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._quantclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; quantclaw-content-hash: ${contentHash}\n${contentBody}`;
}
