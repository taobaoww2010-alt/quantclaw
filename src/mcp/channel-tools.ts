...
import { z } from "zod";
import type { QuantClawChannelBridge } from "./channel-bridge.js";
import {
...
export function registerChannelMcpTools(server: McpServer, bridge: QuantClawChannelBridge): void {
  server.tool(
    "conversations_list",
    "List QuantClaw channel-backed conversations available through session routes.",
    {
  ...
  server.tool(
    "conversation_get",
    "Get one QuantClaw conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one QuantClaw conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one QuantClaw conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued QuantClaw conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued QuantClaw conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same QuantClaw conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open QuantClaw exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending QuantClaw exec or plugin approval request.",
    {
