...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return fs
    ...
    .flatMap((dirName) => {
      ...
      const manifestPath = path.join(rootDir, "quantclaw.plugin.json");
...
describe("bundled plugin naming guardrails", () => {
  it.each([
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match quantclaw.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled quantclaw.install.npmSpec aligned with the package name",
      message:
        "Bundled quantclaw.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled quantclaw.channel.id values must match quantclaw.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
