...
import { discoverQuantClawPlugins, type PluginCandidate } from "./discovery.js";
import {
  loadPluginManifest,
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.QUANTCLAW_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.QUANTCLAW_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: QuantClawPackageManifest["channel"];
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "quantclaw",
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "quantclaw") === "bundle";
    const manifestRes:
        ...
        message:
          minHostVersionCheck.kind === "invalid"
            ...
            : minHostVersionCheck.kind === "unknown_host_version"
              ? `plugin requires QuantClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
              : `plugin requires QuantClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
      });
      continue;
    }
