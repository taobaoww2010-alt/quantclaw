...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantclaw-marketplace-"));
...
> {
  ...
  try {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantclaw-marketplace-download-"));
