 ...
 */
export type QuantClawPluginConfigSchema = {
...
export type QuantClawPluginToolContext = {
...
export type QuantClawPluginToolFactory = (
  ctx: QuantClawPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type QuantClawPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type QuantClawPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<QuantClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). QuantClaw uses this to expose
 ...
 *
 * Runs after QuantClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common QuantClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and QuantClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before QuantClaw builds its generic
...
/**
 * Provider-owned stream wrapper hook after QuantClaw applies its generic
 ...
 *
 * Return `true` or `false` to override QuantClaw's built-in provider cache TTL
 ...
 *
 * Runs only after QuantClaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that QuantClaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after QuantClaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, QuantClaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * QuantClaw calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic QuantClaw wrappers.
   *
   ...
   *
   * Called after QuantClaw resolves the raw configured credential but before the
   ...
   * hint than QuantClaw's generic auth-store guidance.
   */
   ...
   * QuantClaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when QuantClaw should surface a provider-specific hint for
   ...
   * merging. QuantClaw deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * QuantClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `quantclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `quantclaw doctor` should offer.
   *
   ...
   *
   * QuantClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep QuantClaw's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
   ...
   * when QuantClaw has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: QuantClawConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: QuantClawConfig, value: unknown) => void;
  applySelectionConfig?: (config: QuantClawConfig) => QuantClawConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => QuantClawConfig | Promise<QuantClawConfig>;
...
export type QuantClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current QuantClaw configuration */
  config: QuantClawConfig;
 ...
 */
export type QuantClawPluginCommandDefinition = {
...
export type QuantClawPluginHttpRouteAuth = "gateway" | "plugin";
export type QuantClawPluginHttpRouteMatch = "exact" | "prefix";

export type QuantClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type QuantClawPluginHttpRouteParams = {
  path: string;
  handler: QuantClawPluginHttpRouteHandler;
  auth: QuantClawPluginHttpRouteAuth;
  match?: QuantClawPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type QuantClawPluginCliContext = {
  program: Command;
  config: QuantClawConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type QuantClawPluginCliRegistrar = (ctx: QuantClawPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want QuantClaw to keep a plugin command lazy-loaded while still
 ...
 */
export type QuantClawPluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type QuantClawPluginServiceContext = {
  config: QuantClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type QuantClawPluginService = {
  id: string;
  start: (ctx: QuantClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: QuantClawPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether QuantClaw should inject bundle MCP config for this backend.
   *
...
export type QuantClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type QuantClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: QuantClawPluginApi) => void | Promise<void>;
  activate?: (api: QuantClawPluginApi) => void | Promise<void>;
};

export type QuantClawPluginModule =
  | QuantClawPluginDefinition
  | ((api: QuantClawPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

/** Main registration API injected into native plugin entry files. */
export type QuantClawPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | QuantClawPluginToolFactory,
    opts?: QuantClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: QuantClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: QuantClawPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: QuantClawPluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: QuantClawPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, QuantClaw can keep
  ...
  registerService: (service: QuantClawPluginService) => void;
   ...
   */
  registerCommand: (command: QuantClawPluginCommandDefinition) => void;
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * QuantClaw does not await this callback before allowing or denying the tool call.
     */
...
export type PluginHookBeforeInstallPlugin = {
  /** Canonical plugin id QuantClaw will install under. */
  pluginId: string;
