...
function listAgents(cfg: QuantClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: QuantClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<QuantClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: QuantClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: QuantClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: QuantClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<QuantClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  QuantClawConfig,
  {
    bindingsRef: QuantClawConfig["bindings"];
    agentsRef: QuantClawConfig["agents"];
    sessionRef: QuantClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: QuantClawConfig): Map<string, ResolvedAgentRoute> {
