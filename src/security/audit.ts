...
import type { ConfigFileSnapshot, QuantClawConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: QuantClawConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your QuantClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your QuantClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.QUANTCLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.QUANTCLAW_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("quantclaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.QUANTCLAW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.QUANTCLAW_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: QuantClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: QuantClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: QuantClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: QuantClawConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("quantclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("quantclaw security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set QUANTCLAW_GATEWAY_TOKEN/QUANTCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("quantclaw security audit --deep")}".`,
    });
  }
