...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set QUANTCLAW_GATEWAY_TOKEN/QUANTCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "quantclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.QUANTCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.QUANTCLAW_GATEWAY_PASSWORD);
