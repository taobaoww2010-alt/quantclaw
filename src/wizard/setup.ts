...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      "安全警告 — 请阅读",
      "",
      "☯️ QuantClaw is an A-share quantitative trading platform. Trade responsibly.",
      "By default, ☯️ QuantClaw is a personal trading assistant: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "☯️ QuantClaw is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run ☯️ QuantClaw.",
      ...
      "",
      "Run regularly:",
      "quantclaw security audit --deep",
      "quantclaw security audit --fix",
      "",
      "Must read: https://docs.quantclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("☯️ QuantClaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: QuantClawConfig = snapshot.valid
    ? snapshot.exists
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.quantclaw.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("quantclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("quantclaw doctor")}`,
        `Inspect: ${formatCliCommand("quantclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("quantclaw configure")}.`;
  ...
  let localGatewayToken = process.env.QUANTCLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.QUANTCLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: QuantClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
