...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "usage") {
    await loadUsage(host as unknown as QuantClawApp);
  }
  ...
  if (host.tab === "agents") {
    ...
    if (agentId) {
      ...
      if (host.agentsPanel === "files") {
        void loadAgentFiles(host as unknown as QuantClawApp, agentId);
      }
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as QuantClawApp;
...
async function loadOverviewLogs(host: QuantClawApp) {
...
function buildAttentionItems(host: QuantClawApp) {
