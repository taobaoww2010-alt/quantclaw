const SETTINGS_KEY_PREFIX = "quantclaw.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "quantclaw.control.settings.v1";
const LEGACY_TOKEN_SESSION_KEY = "quantclaw.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "quantclaw.control.token.v1:";
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("quantclaw.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
